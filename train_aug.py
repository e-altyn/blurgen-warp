...
from utils.utils import get_env, save_ckpt, calculate_psnr
...
def validate_epoch(model, comp_net, val_loader, loss_fn, metrics, data_range, device):
    model.eval()
    comp_net.eval()

    ssim_metric, lpips_metric = metrics
    ...
    with torch.no_grad():
        for sample in val_loader:
            ...
            psnr_sum += calculate_psnr(pred, blur_img, data_range)
    ...
    val_psnr = sync_tensor(psnr_sum / len(val_loader))
...
def log_validation(epoch, num_epochs, val_loss, psnr, val_ssim, val_lpips, elapsed_time, rank, loss_vals):
    if rank == 0:
        print(f"[Val {epoch}/{num_epochs}] Loss: {val_loss:.4f} | PSNR: {psnr:.2f} | LPIPS: {val_lpips:.4f} | SSIM: {val_ssim:.4f}] Time: {elapsed_time:.2f}")
...
def train_aug(model, optimizer, scheduler, params, 
              ...
              num_poses, val_freq, save_dir, num_epochs, metrics, data_range):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch % val_freq == 0:
            ...
            val_loss, psnr, val_ssim, val_lpips, loss_vals = validate_epoch(
                model, comp_net, val_loader, loss_fn, metrics, data_range, device
            )
            log_validation(
